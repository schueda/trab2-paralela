...
def main():
    ...
    for input_size in input_sizes:
        for word_size in word_sizes:
            ...
            file = f"validations/validationSEQ_{input_size}_{word_size}.txt"
            with open(file, 'r') as f:
                sizes = [float(line.strip().split()[0]) for line in f]
                std = np.std(sizes)
                average = np.mean(sizes)
                row.append(f"{average:.2f} ± {std:.2f}")
            
            for thread in threads:
                file = f"validations/validation{thread}_{input_size}_{word_size}.txt"
                with open(file, 'r') as f:
                    sizes = [float(line.strip().split()[0]) for line in f]
                    std = np.std(sizes)
                    average = np.mean(sizes)
